...
def train(
    ...
    folder_path: str,
    experiment_num: int,
    scenarios_checkpoint: TrainingScenarios = None,
    restart_point: Tuple = None,
):
    ...
    for i, (data_params, data_record) in enumerate(data.items()):
        ...
        train_loader = DataLoader(
            TensorDataset(data_record.x_train, data_record.y_train),
        ...
        val_loader = DataLoader(
            TensorDataset(data_record.x_val, data_record.y_val),
            batch_size=config["batch_size"],
            shuffle=False,
            **kwargs,
        )
        ...
        for model_name in models:
            ...
            if model_name not in scenarios["training_records"][data_params].keys():
                ...
                avg_acc = scenarios["training_records"][data_params][model_name][
                    "avg_acc"
                ]
                avg_test_acc = scenarios["training_records"][data_params][model_name][
                    "avg_test_acc"
                ]
                test_accs = scenarios["training_records"][data_params][model_name][
                    "test_accs"
                ]
            records = []
            for j in range(config["runs_per_model"]):
                if (
                    restart_point != None
                    and model_name == restart_point[0]
                    and restarted == False
                ):
                    if j < restart_point[1]:
                        ...
                        restarted = True
                    print(
                        f"Restarting training for {data_params} experiment {experiment_num} at {model_name} run {j+1}"
                    )
                ...
                for epoch in tqdm(range(config["epochs"])):
                    ...
                    for x, y in val_loader:
                        ...
                        epoch_validation_accuracies = np.append(
                            epoch_validation_accuracies, acc
                        )
                        del x
                        del y
                        del acc
                    ...
                    #    break
                    validation_accuracies = np.append(
                        validation_accuracies,
                        sum(epoch_validation_accuracies)
                        / len(epoch_validation_accuracies),
                    )
                ...
                if "CNN" in model_name:
                    test_data = data_record.x_test.reshape(
                        data_record.x_test.shape[0], 1, edge_length, edge_length
                    )

                otpt = save_model(test_data.float())

                torch_inds = data_record.y_test.detach().numpy() == np.argmax(
                    otpt.detach().numpy(), axis=1
                )
                test_acc = test_accuracy(
                    torch.argmax(otpt, dim=1), data_record.y_test
                ).item()
                ...
                del test_acc
                save_path = (
                    f"{folder_path}/{data_params}_{model_name}_{experiment_num}_{j}.pt"
                )
                torch.save(save_model, save_path)

                train_record = TrainingRecord(
                    f"{data_params}_{model_name}_{j}",
                    save_path,
                    save_path,
                    training_loss,
                    validation_loss,
            ...
            accs["test"][data_params][model_name] = (
                avg_test_acc / config["runs_per_model"]
            )
            print(accs["test"][data_params])
            scenarios["training_records"][data_params][model_name]["avg_acc"] = (
                avg_acc / config["runs_per_model"]
            )
            scenarios["training_records"][data_params][model_name]["avg_test_acc"] = (
                avg_test_acc / config["runs_per_model"]
            )
            scenarios["training_records"][data_params][model_name]["test_accs"] = (
                test_accs
            )
...
def main():
    ...
    if len(data_scenario) == 0:
        raise Exception(
            f"Scenario {sys.argv[1]} not recognised! Check the specified data_path in training_config.json and try again"
        )
    ...
    scenarios = train(
        ...
        scenarios_checkpoint=scenarios_checkpoint,
