...
def precision(gt_mask, attribution, n_dim=64, n=8):
    ...
    overlapping = np.intersect1d(
        inds_gt, inds_attr
    )  # precision = overlap between n most important pixels and n ground truth pixels
...
def top_8_score(combined_mask, attributions, n_dim=64):
    ...
    for i in range(length):
        try:
            prec_score, data_mask = precision(
                combined_mask.reshape(n_dim),
                attributions[i].reshape(n_dim),
                n_dim=n_dim,
                n=n,
            )
            ...
            continuous_emd_score = continuous_emd(
                normalised_gt.reshape((edge_length, edge_length)),
                normalised_attribution.reshape((edge_length, edge_length)),
                n_dim=n_dim,
            )
    ...
    for model in [model_name]:
        ...
        for i in [model_ind]:  # len(xai_output[model]) == 5
            ...
            for method_name, attributions in xai_output[model][i].items():
                ...
                correct_attributions = np.array(attributions)[:test_size].reshape(
                    (test_size, n_dim)
                )[correct_inds[:test_size]]
                correct_masks = masks_test[correct_inds[:test_size]]

                incorrect_attributions = np.array(attributions)[:test_size].reshape(
                    (test_size, n_dim)
                )[incorrect_inds[:test_size]]
                ...
                for ind in range(0, test_size, mini_batch):
                    ...
                    precision_4, _, continuous_emd_scores_4 = top_4_score(
                        correct_masks[ind : ind + mini_batch], correct_batch, n_dim
                    )
                    precision_incorrect_4, _, continuous_emd_scores_incorrect_4 = (
                        top_4_score(
                            incorrect_masks[ind : ind + mini_batch],
                            incorrect_batch,
                            n_dim,
                        )
                    )
                    ...
                    if "translations" in scenario:
                        ...
                        continuous_emd_scores_incorrect = (
                            continuous_emd_scores_incorrect_4
                        )

                    else:
                        precision, _, continuous_emd_scores = top_8_score(
                            top_8_mask, correct_batch, n_dim
                        )
                        precision_incorrect, _, continuous_emd_scores_incorrect = (
                            top_8_score(top_8_mask, incorrect_batch, n_dim)
                        )
                    ...
                    correct_dict["background"] += [scenario.split("_")[-1]] * len(
                        precision
                    )
                    ...
                    incorrect_dict["background"] += [scenario.split("_")[-1]] * len(
                        precision_incorrect
                    )
