...
def main():
    ...
    if config["num_experiments"] > 1:
        experiments_regex = ""
    torch_model_paths = glob(
        f'{config["training_path"]}/{sys.argv[1]}*_{sys.argv[2]}*{experiments_regex}.pt'
    )

    with open(
        f'{config["xai_path"]}/{sys.argv[1]}_{sys.argv[2]}_xai_records.pkl', "rb"
    ) as file:
    ...
    for model_name in ["LLR", "MLP", "CNN"]:
        ...
        for model_path in torch_model_paths:
            if model_name in model_path:
                ...
                if model_name == "CNN":
                    test_data_torch = test_data_torch.reshape(
                        test_data_torch.shape[0], 1, edge_length, edge_length
                    )
                torch_otpt = model(test_data_torch.float())
                torch_inds = data[scenario].y_test.detach().numpy() == np.argmax(
                    torch_otpt.detach().numpy(), axis=1
                )
                ...
                args_list.append(
                    [
                        xai_output[scenario],
                        scenario,
                        data[scenario].masks_test[:test_size],
                        test_size,
                        model_name,
                        model_ind,
                        config["mini_batch"],
    ...
    dump_as_pickle(
        data=quantitative_result,
        output_dir=out_folder_path,
        file_name=f"{sys.argv[1]}_{sys.argv[2]}_quantitative_results",
    )
