...
from xai_master.scenarios.xai_tris.xai_tris_repo.common import DataRecord, SEED
...
def get_background(
    bg_type: str, base: np.ndarray, imagenet: np.ndarray, config: Dict, shape: tuple
) -> np.ndarray:
    if bg_type == "correlated":
        return np.array(
            [
                gaussian_filter(
                    np.reshape(bg, shape), config["smoothing_sigma"]
                ).reshape(-1)
                for bg in base
            ]
        )
...
def combine_inputs(signal, background, manipulation_type, alpha, distractor=None):
    ...
    if manipulation_type == "multiplicative":
        ...
        if not isinstance(alpha, list) or len(alpha) != 3:
            raise ValueError(
                f"Expected list of 3 alphas for distractor_additive, got {alpha}"
            )
        ...
        if not isinstance(alpha, list) or len(alpha) != 3:
            raise ValueError(
                f"Expected list of 3 alphas for distractor_multiplicative, got {alpha}"
            )
        a_s, a_d, a_b = alpha
        signal = 1 - a_s * signal
        ...
        if not isinstance(alpha, list) or len(alpha) != 3:
            raise ValueError(
                f"Expected list of 3 alphas for distractor_division, got {alpha}"
            )
        a_s, a_d, a_b = alpha
        signal = a_s * signal
        ...
        return (
            (a_b * background)
            / (a_s * signal + (1 - a_s) + 5e-9)
            / (a_d * distractor + (1 - a_d) + 5e-9)
        )
                ...
                for manip_type, snr_by_bg in params["snrs"].items():
                    for bg_type in background_types:
                        ...
                        if bg_type in ["white", "correlated"]:
                            background_data = get_background(
                                bg_type,
                                base_backgrounds,
                                imagenet_backgrounds,
                                config,
                        ...
                        if isinstance(snr_by_bg, dict):
                            ...
                            if alpha_set is None:
                                print(
                                    f"No SNRs defined for {param_name} / {manip_type} / {bg_type}"
                                )
                                continue
                        elif isinstance(snr_by_bg, list):
                            alpha_set = (
                                snr_by_bg  # assume this is directly the list of alphas
                            )
                        else:
                            raise ValueError(
                                f"Unexpected snr_by_bg type for {param_name} / {manip_type}: {type(snr_by_bg)}"
                            )
                        ...
                        for alpha in alpha_set:
                            if "distractor" in manip_type and not (
                                isinstance(alpha, list) and len(alpha) == 3
                            ):
                                raise ValueError(
                                    f"Expected list of 3 alphas for {manip_type}, got {alpha}"
                                )
                            ...
                            y = torch.ravel(
                                torch.cat(
                                    (
                                        torch.zeros(config["sample_size"] // 2, 1),
                                        torch.ones(config["sample_size"] // 2, 1),
                                    )
                                )
                            )
                            ...
                            x_val, y_val = (
                                x[val_test_idx][val_idx],
                                y[val_test_idx][val_idx],
                            )
                            x_test, y_test = (
                                x[val_test_idx][test_idx],
                                y[val_test_idx][test_idx],
                            )
                            masks_val, masks_test = (
                                ground_truths[val_test_idx][val_idx],
                                ground_truths[val_test_idx][test_idx],
                            )

                            alpha_str = (
                                "_".join([f"{a:.2f}" for a in alpha])
                                if isinstance(alpha, list)
                                else f"{alpha:.2f}"
                            )
                            ...
                            # Save as torch dict
                            torch.save(s, f"{output_dir}/{scenario_key}.pt")
...
def main():
    ...
    output_path = (
        os.path.join(config["output_dir"], output_override)
        if output_override
        else os.path.join(
            config["output_dir"], datetime.now().strftime(f"%Y-%m-%d-%H-%M-{mode}")
        )
    )
