...
def generate_backgrounds(
    sample_size: int, mean_data: int, var_data: float, image_shape: List[int]
) -> np.ndarray:
...
def generate_xor(params: Dict, image_shape: List[int]) -> np.ndarray:
    ...
    for k, (i_start, signs) in enumerate(zip(range(0, N, N // 4), combos)):
        ...
        for i in range(i_start, i_start + N // 4):
            out[i][
                poses[0][0] : poses[0][0] + p1.shape[0],
                poses[0][1] : poses[0][1] + p1.shape[1],
            ] = p1
            out[i][
                poses[1][0] : poses[1][0] + p2.shape[0],
                poses[1][1] : poses[1][1] + p2.shape[1],
            ] = p2
            if params["pattern_scale"] > 3:
...
def generate_two_distractors(
    ...
    for j in range(N):
        ...
        patterns[j][
            pos1[0] : pos1[0] + pat1.shape[0], pos1[1] : pos1[1] + pat1.shape[1]
        ] = pat1 * rand_signs[0]
        patterns[j][
            pos2[0] : pos2[0] + pat2.shape[0], pos2[1] : pos2[1] + pat2.shape[1]
        ] = pat2 * rand_signs[1]
